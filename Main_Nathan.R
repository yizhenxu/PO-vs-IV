...
source("theme_min.r")
#####################################
## BayesSurvival wrapper functions ##
#####################################
...
plot_cdf_dat <- function(pcuts, pH, labels = c("IV", "Oral"), alab, wlab) {
  cdf_summaries <- lapply(seq_along(pcuts), function(i) {
    summarize_cdf_steps(pcuts[[i]], pH[[i]], labels[i])
  })
  ...
  res = cbind(step_mean, step_band[, 2:3])
  
  ### calculate difference
  cut_vec = pcuts[[1]]
  times <- c(0, cut_vec, max(cut_vec)+1)
  
  H_mat = pH[[1]]; H_ext <- cbind(0, H_mat); S_draws <- exp(-H_ext); F1 <- 1 - S_draws
  H_mat = pH[[2]]; H_ext <- cbind(0, H_mat); S_draws <- exp(-H_ext); F2 <- 1 - S_draws
  F_draws <- F2-F1
  ...
  F_lower = surv.pm.at.grid - surv.radius
  F_upper = surv.pm.at.grid + surv.radius
  
  step_mean  <- to_step(eval.vec, surv.pm.at.grid)[, group := "PO-IV"]
  step_lower <- to_step(eval.vec, F_lower)
  step_upper <- to_step(eval.vec, F_upper)
  ...
  res = rbind(res, cbind(step_mean, step_band[, 2:3]))
...
mainDT <- as.data.table(AData)[, .(study_id,
								   Assignment=treatAssignEng,
								   Treatment=fifelse(TrtActual==0, "PO", "IV"),
								   `Follow-up time (days)`=maxFUTimeAllVisits,
								   NORtripsU,
								   `Number of debridements`=n_debridements,
								   `Gram-negative infection`=as.logical(gram_neg),
								   `Gram-positive infection`=as.logical(gram_positive),
								   `Implants retained`=as.logical(fixation_retained),
								   `Age (years)` = age,
								   `BMI (kg/m^2)` = bmi,
								   `Index hospitalization length of stay (days)`=index_hosp_duration,
								   `Excellent or very good health (VR-12)`=as.logical(vr12_q1_exvg),
								   `Received rifampin`=as.logical(BasePO_Rifampin),
								   `Has insurance`=as.logical(insStatus),
								   `AO/OTA type-C fracture`=as.logical(fractureC),
								   `Non-white race/ethnicity`=as.logical(NonWhiteRaceEthnicity),
								   `Male sex`=as.logical(Male),
								   Unemployed=!as.logical(working),
								   `High-school or lower education`=!as.logical(college),
								   `Has kidney disease`=as.logical(renal_disease_indicator),
								   `Has substance abuse`=as.logical(subabuse_indicator),
								   `Has diabetes`=as.logical(diabetes),
								   `Time from fixation to debridement (days)`=fixationToDebridement)
]

t1vars <- c("Follow-up time (days)", "Number of debridements", "Gram-negative infection", "Gram-positive infection", "Implants retained", "Age (years)", "BMI (kg/m^2)", "Index hospitalization length of stay (days)", "Excellent or very good health (VR-12)", "Received rifampin", "Has insurance", "AO/OTA type-C fracture", "Non-white race/ethnicity", "Male sex", "Unemployed", "High-school or lower education", "Has kidney disease", "Has substance abuse", "Has diabetes", "Time from fixation to debridement (days)")
nonnormal <- c("Follow-up time (days)", "Number of debridements", "Index hospitalization length of stay (days)", "Time from fixation to debridement (days)")
exactVars <- c("Gram-negative infection", "Gram-positive infection", "Implants retained", "Excellent or very good health (VR-12)", "Received rifampin", "Has insurance", "AO/OTA type-C fracture", "Non-white race/ethnicity", "Male sex", "Unemployed", "High-school or lower education", "Has kidney disease", "Has substance abuse", "Has diabetes")

exactWrapper <- function(x, y=NULL) {
	if (!is.null(y)) {
		data <- table(x,y)
	} else {
		data <- x
	}
	tryCatch({exact.test(data, to.plot=F)},
		error=function(e) {
			warning("exact.test failed: ", e$message)
		})
}
mainDT[,exactWrapper(`Male sex`, Treatment)]$p.value
mainDT[,fisher.test(`Male sex`, Treatment)]$p.value
t2itt <- CreateTableOne(mainDT, vars=t1vars, strata="Assignment", testExact="exactWrapper", test=F)
t2pp <- CreateTableOne(mainDT[Assignment==Treatment], vars=t1vars, strata="Assignment", testExact="exactWrapper", test=F)
t2at <- CreateTableOne(mainDT, vars=t1vars, strata="Treatment", testExact=exactWrapper, test=F)
if (writeOutput) {
	write.csv(print(t2itt, nonnormal=nonnormal, exact="", quote=F, noSpaces=T), file="t2itt.csv")
	write.csv(print(t2pp, nonnormal=nonnormal, exact="", quote=F, noSpaces=T), file="t2pp.csv")
	write.csv(print(t2at, nonnormal=nonnormal, exact="", quote=F, noSpaces=T), file="t2at.csv")
}

mainDT[,.(sum(`Follow-up time (days)`<365), mean(`Follow-up time (days)`<365)), by=Assignment]
mainDT[Assignment==Treatment,.(sum(`Follow-up time (days)`<365), mean(`Follow-up time (days)`<365)), by=Assignment]
mainDT[,.(sum(`Follow-up time (days)`<365), mean(`Follow-up time (days)`<365)), by=Treatment]
...
if(oncluster){
  wt_mat <- readRDS(paste0(datapath, "weights/mITT_Weights20250912_11.Rds"))
} else {
  wt_mat <- readRDS(paste0(datapath,"weights/mITT_Weights20250912_11.Rds"))
}
...
hazdt <- as.data.table(t(cumhaz[posterior_idx,1:length(cutpoints)]))
hazdt[,`:=`(cutpoint=cutpoints, trtmnt="IV", analysis="ITT", weighted=F)]
hazdt <- melt(hazdt, measure.vars=1:1001, value.name="cumhaz", variable.name="sample")
rm(cumhaz)
...
tmpdt <- as.data.table(t(cumhaz[posterior_idx,1:length(cutpoints)]))
tmpdt[,`:=`(cutpoint=cutpoints, trtmnt="PO", analysis="ITT", weighted=F)]
hazdt <- rbind(hazdt, melt(tmpdt, measure.vars=1:1001, value.name="cumhaz", variable.name="sample"))
rm(tmpdt)
rm(cumhaz)
...
cdfdt = plot_cdf_dat(pcuts, pH, labels = c("IV", "Oral"), alab = "ITT", wlab = F)
...
set.seed(2025)
if(rerun){
  load(paste0(datapath, "mod_post_mITT.RData"))
...
tmpdt <- as.data.table(t(cumhaz[posterior_idx,1:length(cutpoints)]))
tmpdt[,`:=`(cutpoint=cutpoints, trtmnt="IV", analysis="ITT", weighted=T)]
hazdt <- rbind(hazdt, melt(tmpdt, measure.vars=1:1001, value.name="cumhaz", variable.name="sample"))
rm(tmpdt)
rm(cumhaz)
...
tmpdt <- as.data.table(t(cumhaz[posterior_idx,1:length(cutpoints)]))
tmpdt[,`:=`(cutpoint=cutpoints, trtmnt="PO", analysis="ITT", weighted=T)]
hazdt <- rbind(hazdt, melt(tmpdt, measure.vars=1:1001, value.name="cumhaz", variable.name="sample"))
rm(tmpdt)
rm(cumhaz)
...
tmpdt = plot_cdf_dat(pcuts, pH, labels = c("IV", "Oral"), alab = "ITT", wlab = T)
cdfdt = rbind(cdfdt, tmpdt)

if(!rerun){
  save(post.samples, file = "/users/yxu2/delete/mod_post_mITT.RData")
}
...
if(oncluster){
  wt_mat <- readRDS(paste0(datapath, "weights/PP_Weights20250426_11.Rds"))
} else {
  wt_mat <- readRDS(paste0(datapath,"weights/PP_Weights20250426_11.Rds"))
}
...
tmpdt <- as.data.table(t(cumhaz[posterior_idx,1:length(cutpoints)]))
tmpdt[,`:=`(cutpoint=cutpoints, trtmnt="IV", analysis="as-treated", weighted=F)]
hazdt <- rbind(hazdt, melt(tmpdt, measure.vars=1:1001, value.name="cumhaz", variable.name="sample"))
rm(tmpdt)
rm(cumhaz)
...
tmpdt <- as.data.table(t(cumhaz[posterior_idx,1:length(cutpoints)]))
tmpdt[,`:=`(cutpoint=cutpoints, trtmnt="PO", analysis="as-treated", weighted=F)]
hazdt <- rbind(hazdt, melt(tmpdt, measure.vars=1:1001, value.name="cumhaz", variable.name="sample"))
rm(tmpdt)
rm(cumhaz)
...
tmpdt = plot_cdf_dat(pcuts, pH, labels = c("IV", "Oral"), alab = "as-treated", wlab = F)
cdfdt = rbind(cdfdt, tmpdt)
...
set.seed(2025)
if(rerun){
  load(paste0(datapath, "mod_post_PP.RData"))
} else {
  post.samples = vector("list",2)
}
...
tmpdt <- as.data.table(t(cumhaz[posterior_idx,1:length(cutpoints)]))
tmpdt[,`:=`(cutpoint=cutpoints, trtmnt="IV", analysis="as-treated", weighted=T)]
hazdt <- rbind(hazdt, melt(tmpdt, measure.vars=1:1001, value.name="cumhaz", variable.name="sample"))
rm(tmpdt)
rm(cumhaz)
...
tmpdt <- as.data.table(t(cumhaz[posterior_idx,1:length(cutpoints)]))
tmpdt[,`:=`(cutpoint=cutpoints, trtmnt="PO", analysis="as-treated", weighted=T)]
hazdt <- rbind(hazdt, melt(tmpdt, measure.vars=1:1001, value.name="cumhaz", variable.name="sample"))
rm(tmpdt)
rm(cumhaz)
...
tmpdt = plot_cdf_dat(pcuts, pH, labels = c("IV", "Oral"), alab = "as-treated", wlab = T)
cdfdt = rbind(cdfdt, tmpdt)

if(!rerun){
  save(post.samples, file = "/users/yxu2/delete/mod_post_PP.RData") # 2161
}
...
if(oncluster){
  wt_mat <- readRDS(paste0(datapath, "weights/PP_nocs_Weights20250912_11.Rds"))
} else {
  wt_mat <- readRDS(paste0(datapath,"weights/PP_nocs_Weights20250912_11.Rds"))
}
...
tmpdt <- as.data.table(t(cumhaz[posterior_idx,1:length(cutpoints)]))
tmpdt[,`:=`(cutpoint=cutpoints, trtmnt="IV", analysis="per-protocol", weighted=F)]
hazdt <- rbind(hazdt, melt(tmpdt, measure.vars=1:1001, value.name="cumhaz", variable.name="sample"))
rm(tmpdt)
rm(cumhaz)
...
tmpdt <- as.data.table(t(cumhaz[posterior_idx,1:length(cutpoints)]))
tmpdt[,`:=`(cutpoint=cutpoints, trtmnt="PO", analysis="per-protocol", weighted=F)]
hazdt <- rbind(hazdt, melt(tmpdt, measure.vars=1:1001, value.name="cumhaz", variable.name="sample"))
rm(cumhaz)
...
tmpdt = plot_cdf_dat(pcuts, pH, labels = c("IV", "Oral"), alab = "per-protocol", wlab = F)
cdfdt = rbind(cdfdt, tmpdt)
...
set.seed(2025)
if(rerun){
  load(paste0(datapath, "mod_post_PP_nocs.RData"))
} else {
  post.samples = vector("list",2)
}
...
tmpdt <- as.data.table(t(cumhaz[posterior_idx,1:length(cutpoints)]))
tmpdt[,`:=`(cutpoint=cutpoints, trtmnt="IV", analysis="per-protocol", weighted=T)]
hazdt <- rbind(hazdt, melt(tmpdt, measure.vars=1:1001, value.name="cumhaz", variable.name="sample"))
rm(tmpdt)
rm(cumhaz)
...
tmpdt <- as.data.table(t(cumhaz[posterior_idx,1:length(cutpoints)]))
tmpdt[,`:=`(cutpoint=cutpoints, trtmnt="PO", analysis="per-protocol", weighted=T)]
hazdt <- rbind(hazdt, melt(tmpdt, measure.vars=1:1001, value.name="cumhaz", variable.name="sample"))
rm(tmpdt)
rm(cumhaz)
...
tmpdt = plot_cdf_dat(pcuts, pH, labels = c("IV", "Oral"), alab = "per-protocol", wlab = T)
cdfdt = rbind(cdfdt, tmpdt)

if(!rerun){
  save(post.samples, file = "/users/yxu2/delete/mod_post_PP_nocs.RData")
}
...
#ggdraw(final)
if (writeOutput) {
	ggsave(paste0(datapath,"final_plot.pdf"), final, width = 15, height = 6)
}

###################################################
## Write posterior mean cumulative risks to file ##
###################################################
# to determine PO-IV we need to linearly interpolate missing values in each...
setkey(hazdt, analysis, weighted, trtmnt, sample, cutpoint)
irregular <- F
if (irregular) {
	hazdtFill <- hazdt[hazdt[, CJ(trtmnt, sample, cutpoint, unique=T), by=.(analysis, weighted)], on=.(analysis, weighted, trtmnt, sample, cutpoint)]
	setkey(hazdtFill, analysis, weighted, trtmnt, sample, cutpoint)
	hazdtFill <- rbind(hazdtFill,
				   	hazdtFill[CJ(analysis,weighted,trtmnt,sample,cutpoint=0,unique=T,sorted=T)])
	hazdtFill[cutpoint==0, cumhaz:=0] # needed for early linear interpolation
	hazdtFill[,cumhaz:=na.approx(cumhaz, cutpoint, na.rm=F), by=.(analysis, weighted, trtmnt, sample)]
	hazdtFill <- hazdtFill[cutpoint!=0]
	
}

hazdt[,cumrisk:=1-exp(-cumhaz)]

hazdt <- rbind(hazdt,
	  			hazdt[,.(trtmnt="PO-IV",
			   			cumrisk=(cumrisk[trtmnt=="PO"]-cumrisk[trtmnt=="IV"]),
			   			cumhaz=(cumhaz[trtmnt=="PO"]-cumhaz[trtmnt=="IV"])),
						by=.(analysis, weighted, cutpoint, sample)]) # NOTE: don't have to use by here
setkey(hazdt, analysis, weighted, trtmnt, sample, cutpoint)
rper <- function(x) { round(x * 100, digits=1) }
postMeanRisk <- hazdt[cutpoint==365,.(`1-year cumulative risk`=
					  paste0(rper(mean(cumrisk)), "% (", 
							 rper(quantile(cumrisk, 0.025)), "% -- ",
							 rper(quantile(cumrisk, 0.975)), "%)")),
		by=.(analysis, weighted, trtmnt)]
t3 <- dcast(postMeanRisk[(analysis=="as-treated" & weighted) | 
	  				(analysis=="ITT" & !weighted) |
					(analysis=="per-protocol" & !weighted)],
				trtmnt ~ analysis + weighted,
				value.var="1-year cumulative risk")
if (writeOutput) {
	fwrite(t3, file="t3.csv")
}
postMeanRisk
...
# save to CSV
if (writeOutput) {
	write_csv(tab_combined, paste0(datapath, "tab_combined.csv"))
}
...
if (writeOut) {
	pdf(paste0(datapath, "surv_and_cdf_band.pdf"), width = 10, height = 9)  # adjust size as needed
	print(surv_grid)
	print(cdf_grid)
	dev.off()
}

source("Main_Nathan_remake_plots.R") # remake cumulative hazard and CDF plots
